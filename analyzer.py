...
def enumerator(cdir, file):
    ...
    backbone = input("What backbone do you want?")
    ...
    for problem in paths.keys():
        ...
        for alg in os.listdir(path):
            ...
            test_path = join(alg_path, f"{backbone}-runs") 
            if os.path.isdir(test_path):
                #process_runs(alg_path)
                files = [join(alg_path, f"{backbone}-runs") for x in os.listdir(join(alg_path,f"{backbone}-runs")) if file in x]     
                for filepath in files:
                    yield f"{backbone}-{alg}", problem, filepath # full path e.g., results/min/N5k1test16/fo-maml/conv4-runs/
...
def process_scores(problem, alg, path, results, cross=False, **kwargs):
    ...
    print(path)
    # Create dataframe from reading all result files from different runs
    if not cross:
        score_files = [join(path, x) for x in os.listdir(path) if "test_scores.csv" in x]
    else:
        score_files = [join(path, x) for x in os.listdir(path) if "cross_scores.csv" in x]

    dfs = [pd.read_csv(x, index_col=0) for x in score_files]
    df = pd.concat(dfs)[:5] # only 5 runs
    if len(df) < 5:
        print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
        print("# WARNING--- LESS THAN 5 RUNS IN RESULT PATH FOR", alg)
        print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
...
def table(cdir):
    ...
    for alg, problem, path in enumerator(cdir, file):
        ...
        fn(problem, alg, path, results, minimize=minimize, cross=cross)
