...
from networks import SineNetwork, Conv4, BoostedConv4, ConvX, ResNet, LinearNet,\
                     ...
                     ResNetPlain, MultiplicativeLSTM, SlotLSTM, OPLSTM, FeedForwardClassifier, FeedForwardClassifierNoBatchNorm #SVDTransformSineNetwork,\
...
from algorithms.maml_distr import DISTR
...
from algorithms.sap_reptile import ReptSAP
from algorithms.simplelstm import SimpleLSTM
from algorithms.simplelstm_image import SimpleImageLSTM
from algorithms.multlstm import MultLSTM
from algorithms.oplstm import OpLSTM
from algorithms.fc_oplstm import FCOpLSTM
from algorithms.slotlstm import MetaSlotLSTM
...
FLAGS.add_argument("--model", choices=["tfs", "finetuning", "centroidft", 
                   "lstm", "maml", "moso", "lstm2", "turtle", "reptile", "tloss", 
                   "spft", "mamlft", "sap", "psap", "boil", "reptsap", "distr",
                   "simplelstm", "sapresnet", "multlstm", "slotlstm", "oplstm", "protonet"], required=True,
...
FLAGS.add_argument("--N_train", type=int, default=None,
                   help="Number of classes when meta-training")   
...
FLAGS.add_argument("--lstm_inputs", choices=["prevtarget", "prev_target_pred", "prev_target_err", "prev_target_pred_err", "target_pred", "target", "currtarget"], default=None, 
                   help="Input type to simpleLSTM")
...
FLAGS.add_argument("--avg_hidden",  default=False, action="store_true",
                   help="average cells state")
...
FLAGS.add_argument("--gacc",  default=False, action="store_true",
                   help="use gradient accumulation to prevent OOM")

FLAGS.add_argument("--dropout", type=float, default=None,
                   help="Dropout rate")

FLAGS.add_argument("--num_slots", type=int, default=5,
                   help="Number of slots")

FLAGS.add_argument("--learn_init_weight", action="store_true", default=False,
                   help="Whether to learn the initial weights")

FLAGS.add_argument("--elwise", action="store_true", default=False,
                   help="Whether to apply OPLSTM element-wise")

FLAGS.add_argument("--softmax", action="store_true", default=False,
                   help="Whether to apply softmax in final layer base-learner OPLSTM")

FLAGS.add_argument("--fcnn", action="store_true", default=False,
                   help="Whether to use fully-connected classifier")

FLAGS.add_argument("--update_bias", action="store_true", default=False,
                   help="Whether to update the bias with the hidden states")

FLAGS.add_argument("--no_batchnorm", action="store_true", default=False,
                   help="no batch norm in feedforward classifier")

FLAGS.add_argument("--analyze_learned", action="store_true", default=False,
                   help="analyze and store the learned behavior of OP-LSTM")
...
def setup(args):
    ...
    if args.k_train is None:
        args.k_train = args.k
    if args.model == "simplelstm" and args.zero_supp.lower() == "true" and not args.lstm_inputs == "currtarget":
    ...
    mod_to_conf = {
        ...
        "distr": (DISTR, MAML_CONF),
        "simplelstm": (SimpleLSTM, MAML_CONF),
        "sapresnet": (SAPResNet, MAML_CONF), 
        "multlstm": (MultLSTM, MAML_CONF),
        "slotlstm": (MetaSlotLSTM, MAML_CONF),
        "oplstm": (OpLSTM, MAML_CONF),
        "protonet": (PrototypicalNetwork, MAML_CONF),
    }

    # use regular LSTM for omniglot too 
    if not "sine" in args.problem and not "poly" in args.problem and not "linear" in args.problem and not args.fcnn:
        mod_to_conf["simplelstm"] = (SimpleImageLSTM, MAML_CONF)
    
    # use FC-OPLSTM if the problem is omniglot or sine wave regression
    # as they use fully-connected neural networks
    if "sine" in args.problem or ("omni" in args.problem and args.fcnn):
        mod_to_conf["oplstm"] = (FCOpLSTM, MAML_CONF)
    ...
    conf["avg_hidden"] = args.avg_hidden
    ...
    conf["gacc"] = args.gacc # gradient accumulation for MAML
    conf["num_slots"] = args.num_slots
    conf["lstm_inputs"] = args.lstm_inputs
    conf["elwise"] = args.elwise
    conf["softmax"] = args.softmax
    conf["update_bias"] = args.update_bias
    conf["analyze_learned"] = args.analyze_learned

    if conf["hyper"]:
        assert conf["final_linear"], "if hyper is True, final_linear must also be True"

    args.rgb = args.rgb.lower() == "true"
    if "omni" in args.problem:
        args.rgb = False 
    ...
    conf["N"] = args.N
    conf["learn_init_weight"] = args.learn_init_weight
    conf["no_batchnorm"] = args.no_batchnorm
    ...
    if args.problem == "linear":
        ...
        if not "sap" in args.model:
            ...
            if args.trans_net:
                ...
                elif args.model == "multlstm":
                    conf["baselearner_fn"] = MultiplicativeLSTM
                elif args.model == "slotlstm":
                    conf["baselearner_fn"] = SlotLSTM
                elif args.model == "oplstm":
                    conf["baselearner_fn"] = SineNetwork #OPLSTM
                    conf["lstm_constructor"] = OPLSTM
                    conf["sine"] = True
                else:
        ...
        conf["baselearner_args"] = {"criterion":nn.MSELoss(), "dev":args.dev}
        if args.model == "simplelstm" or args.model == "multlstm" or args.model == "slotlstm" or args.model == "oplstm":
            if not "omni" in args.problem:
                conf["baselearner_args"]["input_size"] = 2
            else:
                conf["baselearner_args"]["input_size"] = 28**2 + args.N
        ...
        if args.model == "simplelstm":
            ...
            conf["lstm_constructor"] = MultiplicativeLSTM
        elif args.model == "slotlstm":
            conf["lstm_constructor"] = SlotLSTM
        elif args.model == "oplstm":
            conf["lstm_constructor"] = OPLSTM
        ...
        if args.backbone is None:
            if args.model == "centroidft":
                ...
                if "psap" in args.model:
                    ...
                    if "omni" in args.problem and args.model == "simplelstm":
                        if args.fcnn:
                            conf["baselearner_fn"] = GeneralLSTM
                        else:
                            conf["baselearner_fn"] = ConvX
                        assert not (args.num_layers != 1 and args.layers is not None), "num layers must be 1 if using simplelstm on omniglot when layers is specified"
                    elif "omni" in args.problem and args.model == "multlstm":
                        conf["baselearner_fn"] = MultiplicativeLSTM
                    elif "omni" in args.problem and args.model == "slotlstm":
                        conf["baselearner_fn"] = SlotLSTM
                    else: 
                        if "omni" in args.problem and args.fcnn:
                            conf["baselearner_fn"] = FeedForwardClassifier if not args.no_batchnorm else FeedForwardClassifierNoBatchNorm
                            if args.no_batchnorm:
                                print("USING NO BATCHNORM")
                            print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ USING FEEDFORWARD-CLASSIFIER @@@@@@@@@@@@@@@@@@@@@@@@@@@@")
                        else:
                            conf["baselearner_fn"] = ConvX 
                lowerstr = "conv4"
        ...
        if args.model in baselines:
            if not args.model == "tfs":
                ...
                train_classes = (args.N if args.N_train is None else args.N_train) # TFS does not train, so this enforces the model to have the correct output dim. directly
            ...
            if args.special:
                train_classes = 64
                print("NUM CLASSES PER TASK:", (args.N if args.N_train is None else args.N_train))
                train_loader = ds(root="./data/", num_classes_per_task=(args.N if args.N_train is None else args.N_train), meta_train=True, 
                ...
                train_classes = (args.N if args.N_train is None else args.N_train)
                ...
                if args.model == "spft":
                    ...
                    train_loader = ds(root="./data/", num_classes_per_task=(args.N if args.N_train is None else args.N_train), meta_train=True, 
                            meta_val=False, meta_test=False, meta_split="train",
                            transform=train_transform,
                            target_transform=Compose([Categorical((args.N if args.N_train is None else args.N_train))]),
                            download=True)
                    print("using this data loader")
                    if not args.gamma is None and not args.gamma == 0 and not "oplstm" in args.model:
        ...
        if args.model == "simplelstm" or args.model == "multlstm" or args.model == "slotlstm" or args.model == "oplstm":
            if args.fcnn or args.problem == "sine":
                if not "omni" in args.problem:
                    conf["baselearner_args"]["input_size"] = 2
                else:
                    conf["baselearner_args"]["input_size"] = 28**2 + args.N
        ...
        if args.dropout is not None:
            conf["baselearner_args"]["dropout"] = args.dropout
    ...
    create_dir(args.resdir)
    if args.N and args.N_train is None:
        args.resdir += 'N' + str(args.N) + 'k' + str(args.k) + "test" + str(args.k_test) + '/' 
    elif args.N and not args.N_train is None:
        args.resdir += "N_train" + str(args.N_train) + 'N' + str(args.N) + 'k' + str(args.k) + "test" + str(args.k_test) + '/' 
    else:
    ...
    if args.gamma is None or args.model == "sap" or args.model == "psap" or args.model=="sapresnet" or args.model == "oplstm":
...
def body(args, conf, train_loader, val_loader, test_loaders, files, model_constr):
    ...
    if not args.gamma is None and not "sap" in args.model and not "oplstm" in args.model:
    ...
    for run in range(args.runs):
        ...
        if args.model_path is None or args.dotraining:
            ...
            if model.trainable:
                ...
                for el in dcounter:
                    ...
                    history = []
                    if args.sine:
                        train_loader = iter(cycle(args.data_loader.generator(episodic=True, batch_size=args.train_batch_size, reset_ptr=True, mode="train")))
                    ...
                    for eid, epoch in enumerate(train_loader):
                        ...
                        if args.batchmode:
                            if args.linear:
                                ...
                                train_loss = model.train(train_x=train_x, train_y=train_y, test_x=None, test_y=None)
                            else:
                                train_x, train_y = epoch
                                train_loss = model.train(train_x=train_x, train_y=train_y.view(-1), test_x=None, test_y=None)
                        elif custom:
                            ...
                            train_loss = model.train(train_x=train_x[0], train_y=train_y[0], test_x=test_x[0], test_y=test_y[0], 
                                        global_train_y=global_train_y[0], global_test_y=global_test_y[0])
                            #import sys; sys.exit()
                        else:
                            if args.linear:
                                (train_x, train_y, test_x, test_y) = epoch
                                train_loss = model.train(train_x=train_x, train_y=train_y, test_x=test_x, test_y=test_y)
                            else:
                                (train_x, train_y), (test_x, test_y) = epoch['train'], epoch['test']
                                if args.num_aug > 0:
                                    ...
                                    if not args.gamma is None and not "min" in args.problem and not "cub" in args.problem and not "tiered" in args.problem and not "oplstm" in args.model:
                                        train_loss = model.train(train_x=train_x[0], train_y=train_y[0], test_x=test_x[0], test_y=test_y[0], joint_x=joint_x, joint_y=joint_y)
                                    else:
                                        train_loss = model.train(train_x=train_x[0], train_y=train_y[0], test_x=test_x[0], test_y=test_y[0])
                        ...
                        if not train_loss is None:
                            ...
                            train_accuracies.append(train_loss[1])
        ...
        if not args.linear or args.sine:
            for idx, (eval_gen, filename) in enumerate(zip(generators, filenames)):
                ...
                train_loss_path = args.resdir+f"{args.backbone}-train_losses.npy"
                train_acc_path = args.resdir+f"{args.backbone}-train_accs.npy"
                np.save(train_acc_path, train_accuracies)
                np.save(train_loss_path, train_losses)
                
                
                if args.analyze_learned:
                    try:
                        np.save(args.resdir+f"euclid_grad", model.euclid_stats_grad)
                        np.save(args.resdir+f"cosine_grad", model.cosine_stats_grad)
                        np.save(args.resdir+f"euclid_proto", model.euclid_stats_proto)
                        np.save(args.resdir+f"cosine_proto", model.cosine_stats_proto)
                    except Exception as e:
                        print(e)
