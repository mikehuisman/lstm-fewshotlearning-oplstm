...
FLAGS.add_argument("--debug", action="store_true", default=False, help="debug mode")
...
def get_save_paths(resdir, seed):
    fn = "test_scores"
    ...
    seeds = [x.split('/')[-1].split('-')[0][3:] for x in files]
    print(seeds)
    seed_idx = seeds.index(str(seed))
    print("The seeds:", seeds, "found seed idx:", seed_idx)
    ...
    if len(seeds) > 1:
        ...
        return [[p+'-model.pkl' for p in prefixes][seed_idx]]
...
def body(args, conf, real_conf, train_loader, test_loader, model_constr, real_model_constr):
    ...
    save_paths = get_save_paths(args.resdir, args.seed)
    print('\nLOADING', save_paths, end='\n\n') #[args.resdir+x for x in os.listdir(args.resdir) if "model-" in x]
    ...
    print("Number of models:", len(ft_models))
    ...
    for eid, epoch in enumerate(train_loader):
        ...
        if (eid + 1) % test_interval == 0 or args.debug: 
            ...
            for vid, vepoch in enumerate(test_loader):
                ...
                if (vid + 1) * args.train_batch_size >= 10000 or args.debug:  
                    for mid, model in enumerate(ft_models):
        ...
        if (eid + 1) * args.train_batch_size >= 100000 or args.debug:
            break


    # Store full data
    if args.N_train is None:
        sp = f"joint/{problem_specifier}/N{args.N}k{args.k}test{args.k_test}/{args.backbone}/{args.split}-{args.model_spec_save}-full_acc-{args.seed}.np"
        sp_std = f"joint/{problem_specifier}/N{args.N}k{args.k}test{args.k_test}/{args.backbone}/{args.split}-{args.model_spec_save}-full_std-{args.seed}.np"
    else:
        create_dir("./joint")
        create_dir(f"./joint/{problem_specifier}")
        create_dir(f"./joint/{problem_specifier}/N_train{args.N_train}N{args.N}k{args.k}test{args.k_test}")
        create_dir(f"./joint/{problem_specifier}/N_train{args.N_train}N{args.N}k{args.k}test{args.k_test}/{args.backbone}")
        sp = f"joint/{problem_specifier}/N_train{args.N_train}N{args.N}k{args.k}test{args.k_test}/{args.backbone}/{args.split}-{args.model_spec_save}-full_acc.np"
        sp_std = f"joint/{problem_specifier}/N_train{args.N_train}N{args.N}k{args.k}test{args.k_test}/{args.backbone}/{args.split}-{args.model_spec_save}-full_std.np"
    np.save(sp, np.array(AVG_ACC))
    np.save(sp_std, np.array(STDS))
    ...
    if use_alldata:
        ...
        if args.N_train is None:
            ...
            save_path = f"joint/{problem_specifier}/N_train{args.N_train}N{args.N}k{args.k}test{args.k_test}/{args.backbone}/{args.split}-{args.model_spec_save}.json"
        ...
        if args.N_train is None:
            ...
            save_path2 = f"joint/{problem_specifier}/N_train{args.N_train}N{args.N}k{args.k}test{args.k_test}/{args.backbone}/{args.split}-{args.model_spec_save}-fulldata.json"
    ...
    if args.N_train is None:
        save_path = f"joint/{problem_specifier}/N{args.N}k{args.k}test{args.k_test}/{args.backbone}/{args.split}-{args.model_spec_save}-{args.seed}.txt"
        create_dir(f"./joint/{problem_specifier}/N{args.N}k{args.k}test{args.k_test}")
        create_dir(f"./joint/{problem_specifier}/N{args.N}k{args.k}test{args.k_test}/{args.backbone}")
    else:
        save_path = f"joint/{problem_specifier}/N_train{args.N_train}N{args.N}k{args.k}test{args.k_test}/{args.backbone}/{args.split}-{args.model_spec_save}-{args.seed}.txt"
        create_dir(f"./joint/{problem_specifier}/N_train{args.N_train}N{args.N}k{args.k}test{args.k_test}")
        create_dir(f"./joint/{problem_specifier}/N_train{args.N_train}N{args.N}k{args.k}test{args.k_test}/{args.backbone}")
...
if __name__ == "__main__":
    ...
    set_seed(0) # fixed seed, because models will be loaded -> no randomness in models
