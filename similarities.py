...
FLAGS.add_argument("--sim_problem", choices=["min", "cub"], required=True,
...
def setup(args):
    ...
    if args.problem == "linear" or args.problem == "sine":
        ...
        train_loader = train_loader.generator(episodic=episodic, batch_size=args.train_batch_size)
        args.linear = True
    else:
        args.linear = False
...
def body(args, conf, train_loader, val_loader, test_loaders, files, model_constr):
    ...
    for eid, epoch in tqdm(enumerate(test_loaders[0])):
        print(f"Episode {eid}")
        for mid, model in enumerate(models):
