...
class MAML(Algorithm):
    ...
    def __init__(self, train_base_lr, base_lr, second_order, grad_clip=None, meta_batch_size=1, special=False, 
                ...
                train_only_sign_sgd=False, avg_grad=False, train_curve=False, finite_diff=False, **kwargs):
    ...
    def _hessian_vector_product(self, vector, input, target, inner_lr, r=1e-2):
        # r: the small constant in which we move the params
        # vector: gradients w.r.t. base-learner params on query set
        R = r / self._concat(vector).norm()

        init_params = [p.clone().detach() for p in self.initialization]
        for p in init_params: p.requires_grad = True
        
        # add in gradient direction
        # concat flattens all params
        # for p, v in zip(init_params, vector):
        #     p.data.add_(R, v) # adds R *v to each parameter 
        ...
        _, plus_grads = get_loss_and_grads(self.baselearner, input, target, 
                                            ...
                                            retain_graph=False,
                                            flat=False)
        # compute loss with plus weights on training data

        # loss = self.model._loss(input, target) # compute new loss
        # grads_p = torch.autograd.grad(loss, init_params) # loss w.r.t. alphas
        ...
        # grads_n = torch.autograd.grad(loss, init_params)
        ...
        return [z - inner_lr*(x-y).div_(2*R) for x, y, z in zip(plus_grads, min_grads, vector)]
    ...
    def _deploy(self, train_x, train_y, test_x, test_y, train_mode, T, compute_cka=False, var_updates=False):
        ...
        if not self.finite_diff or not train_mode:
            ...
            test_loss, test_grads = get_loss_and_grads(learner, xinp, yinp, 
                                            ...
                                            retain_graph=False,
                                            flat=False)
            estimated_grads = self._hessian_vector_product(test_grads, train_x, train_y, self.base_lr)
