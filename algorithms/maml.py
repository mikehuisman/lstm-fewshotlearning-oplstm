...
class MAML(Algorithm):
    ...
    def __init__(self, train_base_lr, base_lr, second_order, grad_clip=None, meta_batch_size=1, special=False, 
                ...
                special_opt=None, gacc=False, **kwargs):
    ...
    def _deploy(self, train_x, train_y, test_x, test_y, train_mode, T, compute_cka=False, var_updates=False):
        ...
        if not var_updates or train_mode:
            ...
            # put only training data to GPU for now
            if self.gacc:
        ...
        # remove support set from GPU
        # put only training data to GPU for now
        if self.gacc:
            train_x, train_y = put_on_device("cpu", [train_x, train_y])
            #test_x, test_y = put_on_device(self.dev, [test_x, test_y])
        ...
        if (not self.finite_diff and self.special_opt is None) or not train_mode:
            if not self.gacc:
                ...
                MAX_SIZE = 10
                chunks = int(np.ceil(len(yinp)/MAX_SIZE))
                test_loss = 0
                test_preds = []
                for j in range(chunks):
                    xinp, yinp = test_x[j*MAX_SIZE:(j+1)*MAX_SIZE], test_y[j*MAX_SIZE:(j+1)*MAX_SIZE]
                    xinp, yinp = put_on_device(self.dev, [xinp, yinp])
                    preds = learner.forward_weights(xinp, fast_weights)
                    loss = learner.criterion(preds, yinp)*len(yinp)/(len(test_y)*chunks)
                    test_loss += loss.item()
                    if train_mode:
                        loss.backward() 

                    
                    xinp, yinp = put_on_device("cpu", [xinp, yinp])
                    preds = put_on_device("cpu", [preds])[0]
                    test_preds.append(preds)
                    print(test_preds, end='\n\n')
        ...
        if not train_mode: 
            loss_history.append(test_loss.item() if not self.gacc else test_loss)
    ...
    def train(self, train_x, train_y, test_x, test_y):
        ...
        if not self.finite_diff and self.special_opt is None and not self.gacc:
