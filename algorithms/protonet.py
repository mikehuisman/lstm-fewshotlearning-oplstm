...
class PrototypicalNetwork(Algorithm):
    ...
    Computes class prototypes in the support set and classifies
    unseen examples according to the distance to these prototypes.
    ...
    def __init__(self, meta_batch_size=1, **kwargs):
        ...
        **kwargs : dict
        ...
        num_params = sum([p.numel() for p in self.initialization])
    ...
    def _deploy(self, train_x, train_y, test_x, test_y, train_mode):
        ...
        with contxt():
            ...
            # compute prototypes
            prototypes = torch.zeros((num_classes, support_embeddings.size(1)), device=self.initialization[0].device)
            # number of eval_classes = train_classes
            for class_id in range(num_classes):
                mask = train_y == class_id
                prototypes[class_id] = support_embeddings[mask].sum(dim=0) / torch.sum(mask).item()

            # can be seen as negative predictions
            distance_matrix = (torch.cdist(query_embeddings.unsqueeze(0), prototypes.unsqueeze(0)) **2).squeeze(0) 
            predictions = -1 * distance_matrix
            # this is only the prediction loss, we want to add a term that minimizes distance to real class centroid
            loss = self.baselearner.criterion(predictions, test_y) / (num_classes * len(test_y)) 
