...
class MultLSTM(Algorithm):
    ...
    def __init__(self, N, hidden_size, num_layers,  second_order,
                 ...
                 lstm_constructor=None, shuffle=False, learn_hidden=False, **kwargs):
    ...
    def _deploy(self, train_x, train_y, test_x, test_y, train_mode, T):
        ...
        preds = self.lstm.predict(h=output, x_train=torch.cat([train_x, train_onehot_labels], dim=1),
                                  y_train=train_onehot_labels.float(), x_query=test_x) # test x and not 
