...
class SineNetwork(nn.Module):
    ...
    def __init__(self, criterion, in_dim=1, out_dim=1, zero_bias=True, use_tanh=False, hdims=[40,40], **kwargs):
        ...
        **kwargs : dict, optional
        ...
        self.num_weights = len(ls)
        self.architecture = [1] + hdims + [1]
        self.in_features = hdims[-1]
        self.eval_classes = 1
    ...
    def get_fc_weight_params(self):
        params = []
        for b in self.model:
            params.append(b.weight)
        params.append(self.out.weight)
        return params

    def get_fc_bias_params(self):
        params = []
        for b in self.model:
            params.append(b.bias)
        params.append(self.out.bias)
        return params

    def layer_wise_preds(self, x, W, B=None):
        outputs = [x]
        if B is None:
            for i in range(len(self.model)):
                outputs.append(F.relu(F.linear(outputs[-1], W[i], self.model[i].bias)))
            outputs.append(F.linear(outputs[-1], W[-1], self.out.bias))
        else:
            for i in range(len(self.model)):
                outputs.append(F.relu(F.linear(outputs[-1], W[i], B[i])))
            outputs.append(F.linear(outputs[-1], W[-1], B[-1]))
...
class ConvX(nn.Module):
    ...
    def forward_weights(self, x, weights, flat=False, embedding=False):
        """Manual feedforward pass of the network with provided weights
...
class ResNet(nn.Module):
    ...
    def __init__(self, num_blocks, dev, train_classes, eval_classes, criterion=nn.CrossEntropyLoss(), **kwargs):
        super().__init__()
        self.num_blocks = num_blocks
        self.train_classes = train_classes
        self.eval_classes = eval_classes
...
class GeneralLSTM(nn.Module):
    def __init__(self, hidden_size, num_layers, criterion, output_size=1, input_size=1, dev="cpu", final_linear=True, hyper=False, dropout=None, layers=None, **kwargs):
        super().__init__()
        ...
        if layers is None:
            ...
            self.lstm = nn.LSTM(input_size=input_size, hidden_size=hidden_size, num_layers=num_layers, dropout=dropout).to(dev)
            print("BATCH_FIRST:", self.lstm.batch_first)
            ...
            indim = input_size
            outdim = layers[0]
            self.lstms = [nn.LSTM(input_size=indim, hidden_size=outdim, num_layers=1, dropout=dropout).to(dev)]

            for i in range(1,len(layers)):
                indim = outdim
                outdim = layers[i]

                self.lstms.append( nn.LSTM(input_size=indim, hidden_size=outdim, num_layers=1, dropout=dropout).to(dev) )
            self.lstms = nn.ModuleList(self.lstms)

            if self.final_linear:
                self.output = nn.Linear(layers[-1], output_size).to(dev)
            else:
                self.output = nn.LSTM(input_size=layers[-1], hidden_size=output_size, num_layers=1).to(dev)
    ...
    def forward(self, x, prevh=None, prevc=None, prevh_final=None, prevc_final=None):
        if not self.use_layers:
            # prevh_final is the hidden state of the output layer LSTM (if we have it - if self.final_linear=False)
            # idem for prevc final but then the cell state
            ...
            # if we a stacked LSTM with different dimensions
            # prevh is now a list of states for every lstm, like the other arguments

            # x has shape [seq length, batch size, input features]
            # ht has shape [num layers, batch size, hidden dim]
            if prevh is None and prevc is None:
                hts = []; cts = [] # hidden states and cell states
                x, (ht, ct) = self.lstms[0](x)
                hts.append(ht); cts.append(ct)

                for j in range(1, len(self.lstms)):
                    x, (ht, ct) = self.lstms[j](x)
                    hts.append(ht); cts.append(ct)
                ...
                for j in range(0, len(self.lstms)):
                    if noprevh:
                        prevh.append( torch.zeros([self.num_layers, x.size(1), self.lstms[j].hidden_size], device=self.dev) ) # x.size(1): batch size
                    if noprevc:
                        prevc.append( torch.zeros([self.num_layers, x.size(1), self.lstms[j].hidden_size], device=self.dev) ) # x.size(1): batch size

                    x, (ht, ct) = self.lstms[j](x, (prevh[j], prevc[j]))
                    hts.append(ht); cts.append(ct)
...
class MultiplicativeLSTM(nn.Module):
    def __init__(self, hidden_size, num_layers, criterion, output_size=1, input_size=1, dev="cpu", dropout=None, layers=None, learn_hidden=False, **kwargs):
        super().__init__()
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.num_layers = num_layers
        ...
        if layers is None:
            ...
            if self.learn_hidden:
                self.initial_h = nn.Parameter(torch.zeros(self.num_layers, hidden_size)).to(dev)
                self.initial_c = nn.Parameter(torch.zeros(self.num_layers, hidden_size)).to(dev)
            ...
            self.lstms = [nn.LSTM(input_size=indim, hidden_size=outdim, num_layers=1, dropout=dropout).to(dev)]
            if self.learn_hidden:
                self.initial_h = [nn.Parameter(torch.zeros(1, outdim)).to(dev)]
                self.initial_c = [nn.Parameter(torch.zeros(1, outdim)).to(dev)]
            ...
            for i in range(1,len(layers)):
                ...
                self.lstms.append( nn.LSTM(input_size=indim, hidden_size=outdim, num_layers=1, dropout=dropout).to(dev) )
                if self.learn_hidden:
                    self.initial_h.append( nn.Parameter(torch.zeros(1, outdim)).to(dev) )
                    self.initial_c.append( nn.Parameter(torch.zeros(1, outdim)).to(dev) )

            self.lstms = nn.ModuleList(self.lstms)
            if self.learn_hidden:
                self.initial_h = nn.ParameterList(self.initial_h)
                self.initial_c = nn.ParameterList(self.initial_c)


            

        final_hidden_dim = hidden_size if not self.use_layers else layers[-1]
        # self.lstms produce support embeddings

        
        # we also need a writing mask 
        self.mask = nn.Linear(input_size, output_size, bias=True).to(dev) # self.mask.weight will be (output_size, input_size)
        
        # need a weight matrix to map the hidden embeddings onto the same dimensionality as the input 
        # Inputs will be (input_size, 1)
        # Hidden states will be (output_size, hidden_size)
        # M needs to be (hidden_size, input_size)
        # so (output_size, hidden_size) x (hidden_size, input_size) x (input_size, 1) will be (output_size, 1)

        self.P = nn.Linear(input_size-output_size, final_hidden_dim, bias=False).to(dev) # self.P.weight will be of size (final_hidden_dim, input_size)

        # (input size - the output size) because we do not want to attach the 0 label (useless information)
    ...
    def forward(self, x, prevh=None, prevc=None):
        if not self.use_layers:
            ...
            # ht has shape [num layers, batch size, hidden dim]
            if prevh is None and prevc is None:
                if not self.learn_hidden:
                    x, (ht, ct) = self.lstm(x)
                else:
                    x, (ht, ct) = self.lstm(x, (self.initial_h, self.initial_c))
                ...
                if prevh is None:
                    if not self.learn_hidden:
                        ...
                        prevh = self.initial_h
                        
                if prevc is None:
                    if not self.learn_hidden:
                        prevc = torch.zeros([self.num_layers, x.size(1), self.hidden_size], device=self.dev) # x.size(1): batch size
                    else:
                        prevc = self.initial_c
            ...
            if prevh is None and prevc is None:
                if not self.learn_hidden:
                    ...
                    hts = []; cts = []
                    x, (ht, ct) = self.lstms[0](x, (self.initial_h[0], self.initial_c[0]))
                    hts.append(ht); cts.append(ct)
                    
                    for j in range(1, len(self.lstms)):
                        x, (ht, ct) = self.lstms[j](x, (self.initial_j[j], self.initial_c[j]))
                        hts.append(ht); cts.append(ct)
                ...
                for j in range(0, len(self.lstms)):
                    if noprevh:
                        if not self.learn_hidden:
                            prevh = torch.zeros([self.num_layers, x.size(1), self.lstms[j].hidden_size], device=self.dev) # x.size(1): batch size
                        else:
                            prevh = self.initial_h[j]
                            
                    if noprevc:
                        if not self.learn_hidden:
                            prevc = torch.zeros([self.num_layers, x.size(1), self.lstms[j].hidden_size], device=self.dev) # x.size(1): batch size
                        else:
                            prevc = self.initial_c[j]
                            
                    x, (ht, ct) = self.lstms[j](x, (prevh, prevc))
                    hts.append(ht); cts.append(ct)
                
                return x, (hts,cts)

    
    def predict(self, h, y_train, x_train, x_query):

        
        # compute the sigmoid write masks
        # x_train : (N_train, input_size), self.mask.weight: (output_size, input_size)
        #write_masks = torch.sigmoid( self.mask(x_train) ) # (N_train, output_size)
        
        
        # compute outer products of the hidden states for the support examples
        # and the write masks 
        
        # b:batch dim,o:output dim, h: hidden dim
        # write_masks is of shape (batch, output_size)
        # h is of size (1, batch, hidden size)
        # H: (output_size, final hidden_size)
        #H = torch.einsum("bo,bh->boh", write_masks, h.squeeze(0)).sum(dim=0)  
        
        # h.squeeze.T is (hidden, batch) y_train is (batch, output size)
        # H is now (hidden, output)
        H = h.squeeze(0).T @ y_train

        # Map x_query onto d-dimensional space, where d = hidden dim
        x_query = self.P(x_query) # new shape: (batch size, hidden dim)
        
        # multiplicative interaction for perdictions
        
        # print("h:", h,end='\n\n')
        # print("Write mask:", write_masks,end='\n\n')
        # print("x_train", x_train,end='\n\n')
        # print("x_query:", x_query,end='\n\n')
        # print("H:", H,end='\n\n')


        return x_query @ H # (batch dim, output dim)

    # def predict(self, h, x_train, x_query):
    ...
    #     return x_query @ H.T # (batch dim, output dim)
...
class OPLSTM(nn.Module):
    def __init__(self, hidden_size, num_layers, criterion, output_size=1, input_size=1, dev="cpu", dropout=None, 
                 layers=None, learn_hidden=False, learn_init_weight=False, base_input_size=None, param_lr=False, 
                 elwise=False, softmax=False, **kwargs):
        super().__init__()
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.num_layers = num_layers
        ...
        print("INPUT SIZE:", input_size, "OUTPUT SIZE:", output_size, "USING SOFTMAX:", self.softmax)
        ...
        if self.learn_init_weight:
            self.init_weight = nn.Linear(base_input_size, output_size, bias=False).to(dev)

        if self.param_lr:
            self.lr = nn.Parameter(torch.ones(1, device=dev))
        ...
        self.b = nn.Parameter(torch.zeros(final_hidden_dim, device=dev))
        bound = 1/self.input_size
        torch.nn.init.uniform_(self.b, -(bound)**0.5, +(bound)**0.5 )
    ...
    def forward(self, x, prevh=None, prevc=None):
        if self.elwise:
            tsize = x.size(0)
            bsize = x.size(1)
            x = x.reshape(tsize, -1, self.input_size)
        ...
        if not self.use_layers:
            ...
            if prevh is None and prevc is None:
                ...
                # if self.elwise:
                #     x = x.reshape(tsize, bsize, -1)
                #     ht = ht.reshape(ht.size(0), bsize, -1)
                #     ct = ct.reshape(ct.size(0), bsize, -1)
                #print("Still here...")
                return x, (ht,ct)
            else:
                if prevh is None:
    ...
    def predict(self, H, x, B=None):
        # x size: (batch, indim)
        # H size: (outdim, indim)
        #print(x.size(), H.size(), self.b.size())
        if B is None:
            return x @ H.T + self.b 
        else:
            return x @ H.T + B
        
    def init_predict(self, x, H=None, B=None):
        if self.softmax:
            return F.log_softmax(F.linear(x, H, self.b if B is None else B), dim=1)
        return F.linear(x, H, self.b if B is None else B)
...
class SlotLSTM(nn.Module):
    def __init__(self, slot_size, num_slots, input_address_size, input_content_size, **kwargs):
        # slot_size: number of entries/cells in a single slot
        # num_slots: number of slots in the entire cell state/hidden state
        # input_address_size: the dimensionality of the inputs used for addressing
        # input_content_size: the dimensionality of the inputs used to write to memory
        
        super().__init__()
        self.slot_size = slot_size
        self.num_slots = num_slots
        self.num_cells = self.num_slots * self.slot_size
        self.input_address_size = input_address_size
        self.input_content_size = input_content_size
       
        # compute values used for initializing the weights
        init_value_gates = 1/(input_address_size**0.5)
        init_value_content = 1/(input_content_size**0.5)
        
        ############################
        # create weight matrices
        ############################
        # weights and biases for the gates (take as input: address)
        self.Wg = nn.Parameter(torch.zeros(3*num_slots, input_address_size))
        self.bg = nn.Parameter(torch.zeros(3*num_slots))
        
        # weights for the content (take as input: input content)
        self.Wc = nn.Parameter(torch.zeros(slot_size, input_content_size))
        self.bc = nn.Parameter(torch.zeros(slot_size))
        
        # Initialize the weights uniformly using standard rule
        for w in [self.Wg, self.bg]:
            nn.init.uniform_(w, -init_value_gates, +init_value_gates)
        
        for w in [self.Wc, self.bc]:
            nn.init.uniform_(w, -init_value_content, +init_value_content)
            
    
    def forward(self, input_address, input_content, state=None):
        # input_address size: (batch size, input_address_size)
        # input_content zize: (batch size, input_content_size)
        if state is not None:
            prevh, prevc = state
            if prevh is None:
                prevh = torch.zeros(input_address.size(0), self.num_slots, self.slot_size, device=input_address.device)
            if prevc is None:
                prevc = torch.zeros(input_address.size(0), self.num_slots, self.slot_size, device=input_address.device)
        else:
            prevh = torch.zeros(input_address.size(0), self.num_slots, self.slot_size, device=input_address.device)
            prevc = torch.zeros(input_address.size(0), self.num_slots, self.slot_size, device=input_address.device)
        
        # gate weights of size (3*num_slots, input_address size)
        gates = input_address @ self.Wg.T + self.bg # out shape: (batch_size, 3*num_slots) + 
        ft, it, ot = (
            torch.sigmoid(gates[:,:self.num_slots]),
            torch.sigmoid(gates[:,self.num_slots:2*self.num_slots]),
            torch.sigmoid(gates[:,2*self.num_slots:])
        ) # every gate is now of size (batch size, num_slots)
        
        content = torch.tanh( input_content @ self.Wc.T + self.bc ) # shape: (batch_size, slot_size)
        ct = ft.unsqueeze(2) * prevc + torch.einsum('bn,bs->bns', it, content)  # size: (batch size, num_slots, slot_size)
        ht = ot.unsqueeze(2) * torch.tanh(ct) # size: (batch size, num_slots, slot_size)
        
        return ht, (ht, ct)

    def predict(self, h):
        # h is the hidden state of size (batch_size, num_slots, slot_size)
        # we assume that we stored the labels, so that slot_size=N (number of classes)
        # and we can simply sum the labels
        return h.sum(dim=1)
...
class FeedForwardClassifier(nn.Module):
    def __init__(self, dev, eval_classes, img_size=28, criterion=nn.CrossEntropyLoss(), **kwargs):
        super().__init__()
        self.relu = nn.ReLU()
        self.out_dim = eval_classes
        self.eval_classes = eval_classes
        self.train_classes = self.eval_classes
        self.in_dim = img_size[0]**2
        self.dev = dev
        self.in_features = 64
        self.criterion = criterion

        self.architecture = [self.in_dim, 256, "bn", 128, "bn", 64, "bn", 64, "bn", self.out_dim]

        self.lin1 = nn.Linear(self.in_dim, 256)
        ...
        self.out = nn.Linear(64, self.out_dim)
    
    def get_fc_weight_params(self,):
        params = []
        for i in range(1,4+1):
            weight = eval(f"self.lin{i}.weight")
            #bias = eval(f"self.lin{i}.bias")
            params.append(weight)
            #params.append(bias)
        params.append(self.out.weight)
        return params
    ...
    def get_fc_bias_params(self):
        return [self.lin1.bias, self.lin2.bias, self.lin3.bias, self.lin4.bias, self.out.bias]
    ...
    def layer_wise_preds(self, x, W, B=None):
        ...
        l1 = F.linear(x, W[0], b1)
        l1_mean = l1.mean(dim=0); l1_std = (((l1 - l1_mean)**2).sum(dim=0) + 1e-7)**0.5

        running_mean =  torch.zeros(l1.size(1)).to(self.dev)
        running_var = torch.ones(l1.size(1)).to(self.dev)
        l1_post_bn = F.batch_norm(l1, running_mean, running_var, 
                                  self.bn1.weight, self.bn1.bias, momentum=1, training=True)
        l1_post_bn = F.relu(l1_post_bn)


        # layer 2
        l2 = F.linear(l1_post_bn, W[1], b2)
        l2_mean = l2.mean(dim=0); l2_std = (((l2 - l2_mean)**2).sum(dim=0) + 1e-7)**0.5

        running_mean =  torch.zeros(l2.size(1)).to(self.dev)
        running_var = torch.ones(l2.size(1)).to(self.dev)
        l2_post_bn = F.batch_norm(l2, running_mean, running_var, 
                                  self.bn2.weight, self.bn2.bias, momentum=1, training=True)
        l2_post_bn = F.relu(l2_post_bn)

        # layer 3
        l3 = F.linear(l2_post_bn, W[2], b3)
        l3_mean = l3.mean(dim=0); l3_std = (((l3 - l3_mean)**2).sum(dim=0) + 1e-7)**0.5

        running_mean =  torch.zeros(l3.size(1)).to(self.dev)
        running_var = torch.ones(l3.size(1)).to(self.dev)
        l3_post_bn = F.batch_norm(l3, running_mean, running_var, 
                                  self.bn3.weight, self.bn3.bias, momentum=1, training=True)
        l3_post_bn = F.relu(l3_post_bn)

        # layer 4
        l4 = F.linear(l3_post_bn, W[3], b4)
        l4_mean = l4.mean(dim=0); l4_std = (((l4 - l4_mean)**2).sum(dim=0) + 1e-7)**0.5

        running_mean =  torch.zeros(l4.size(1)).to(self.dev)
        running_var = torch.ones(l4.size(1)).to(self.dev)
        l4_post_bn = F.batch_norm(l4, running_mean, running_var, 
                                  self.bn4.weight, self.bn4.bias, momentum=1, training=True)
        l4_post_bn = F.relu(l4_post_bn)

        return [l1,l2,l3,l4,F.linear(l4_post_bn, W[4], bias=bout)], [self.bn1.weight, l1_std, self.bn2.weight, l2_std, self.bn3.weight, l3_std, self.bn4.weight, l4_std]
    ...
    def forward_weights(self, x, weights, embedding=False):
        ...
        for i in range(4):
            x = F.linear(x, weights[4*i], weights[4*i+1])
            running_mean =  torch.zeros(x.size(1)).to(self.dev)
            running_var = torch.ones(x.size(1)).to(self.dev)

            x = F.batch_norm(x, running_mean, running_var, 
                                    weights[4*i+2], weights[4*i+3], momentum=1, training=True)
            x = F.relu(x)
...
class FeedForwardClassifierNoBatchNorm(nn.Module):
    def __init__(self, dev, eval_classes, img_size=28, criterion=nn.CrossEntropyLoss(), **kwargs):
        super().__init__()
        self.relu = nn.ReLU()
        self.out_dim = eval_classes
    ...
    def layer_wise_preds(self, x, W, B=None):
        ...
        # layer 1
        l1 = F.relu(F.linear(x, W[0], b1))
        # layer 2
        l2 = F.relu(F.linear(l1, W[1], b2))
        # layer 3
        l3 = F.relu(F.linear(l2, W[2], b3))
        # layer 4
        l4 = F.relu(F.linear(l3, W[3], b4))

        return [l1,l2,l3,l4,F.linear(l4, W[4], bias=bout)], None
    ...
    def forward_weights(self, x, weights, embedding=False):
        ...
        for i in range(4):
            x = F.linear(x, weights[2*i], weights[2*i+1])
            x = F.relu(x)
